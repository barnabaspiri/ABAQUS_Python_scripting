...
# Import and set the path of the working directory
from parameters import RUNDIR
os.chdir(RUNDIR) 

def demoModel(rectangleSize, youngModulus, displacement):
    try:
        
        # set the job name
        jobName = "demoModel_" + "recSize" + str(round(rectangleSize,1)) + "_youngModulus" + str(round(youngModulus,1)) + "_displacement" + str(round(displacement,1))
        jobName = jobName.replace('.', 'x')
...
for rectangleSize_i in rectangleSize_range:
    for youngModulus_i in youngModulus_range:
        for displacement_i in displacement_range:
            demoModel(rectangleSize_i, youngModulus_i, displacement_i)